              ...
              <el-col :span="12">
                <el-form-item label="类型" prop="challenge_type">
                  <el-select v-model="form.challenge_type" style="width:100%">
                    ...
                    <el-option label="习惯" value="健康习惯" />
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item label="目标指标" prop="target_metric">
                  <el-select v-model="form.target_metric" style="width:100%">
                    <el-option label="体重 (kg)" value="体重" />
                    <el-option label="步数 (步)" value="步数" />
                    <el-option label="睡眠 (小时)" value="睡眠时长" />
                    <el-option label="运动 (分钟)" value="运动时长" />
                  </el-select>
                </el-form-item>
              </el-col>
            </el-row>
            ...
            <el-row :gutter="12">
              ...
              <el-col :span="10">
                <el-form-item label="单位" prop="target_unit">
                  <el-input v-model="form.target_unit" placeholder="如: kg" disabled />
                </el-form-item>
              </el-col>
            </el-row>
        ...
        <el-card class="box-card list-card" shadow="never">
          ...
          <el-table :data="myChallenges" style="width: 100%" v-loading="loading" empty-text="暂无参与的挑战">
            ...
            <el-table-column label="操作" width="100" align="right">
              <template #default="{ row }">
                <el-button type="primary" link icon="Position" @click="openInviteDialog(row)">
                  邀请
                </el-button>
              </template>
            </el-table-column>
...
// 获取挑战列表 (调用 handler.go: GetUserChallenges)
const fetchMyChallenges = async () => {
  if (!userStore.userId) return
  loading.value = true
  try {
    // 对应后端: GET /users/:id/challenges
...
// 创建挑战 (调用 handler.go: CreateChallenge + JoinChallenge)
    ...
    if (valid) {
      ...
      try {
        ...
        const payload = {
          ...
          start_date: form.dateRange[0], // YYYY-MM-DD
          end_date: form.dateRange[1],   // YYYY-MM-DD
          max_participants: 50,
          is_public: true
        }
        ...
        if (res && res.challenge_id) {
          // 3. 关键步骤：因为后端 CreateChallenge 不会自动添加参与记录，
          // 这里必须手动调用 join 接口，否则用户在列表里看不到自己创建的挑战
          await request.post(`/challenges/${res.challenge_id}/join`, {
...
const sendInvite = async () => {
  ...
  inviting.value = true
  try {
    // 对应后端: POST /challenges/:id/invite
...
const getTagType = (type) => {
  const map = { '减重': 'danger', '运动': 'primary', '健康习惯': 'success', '饮食': 'warning' }
